...
class PufferGPUDrive:
    def __init__(self, device="cuda", max_cont_agents=64, num_worlds=64, k_unique_scenes=1):
        ...
        self.observation_space = gymnasium.spaces.Box(low=0, high=255, shape=(self.obs_size,), dtype=np.float32)
        ...
        self.actions = torch.zeros((num_worlds, max_cont_agents), dtype=torch.int64).to(self.device)
    ...
    def step(self, action):
        ...
        done_worlds = torch.where(
            (terminal.nan_to_num(0) * self.controlled_agent_mask).sum(dim=1) == self.controlled_agent_mask.sum(dim=1)
        )[0].cpu()
