    ...
    return gymnasium.spaces.Dict(
        {
            ...
            "action_mask": gymnasium.spaces.Box(low=0, high=1, shape=(action_space(state).n,), dtype=np.int8),
        }
    )
...
def get_obs_and_infos(state):
    ...
    state.last_obs = {
        player: {
            "obs": np.reshape(state.state.observation_tensor(), [-1]).astype(np.float32),
...
def solve_chance_nodes(state):
    ...
    while state.state.is_chance_node():
        ...
        actions, probs = zip(*state.state.chance_outcomes(), strict=False)
