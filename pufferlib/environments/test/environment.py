...
DEFAULT_OBSERVATION_SPACE = gymnasium.spaces.Box(low=-(2**20), high=2**20, shape=(1,), dtype=np.float32)
...
class GymnasiumTestEnv(gym.Env):
    def __init__(self, observation_space=DEFAULT_OBSERVATION_SPACE, action_space=DEFAULT_ACTION_SPACE):
...
class PettingZooTestEnv(ParallelEnv):
    ...
    def reset(self, seed=None):
        ...
        obs = {a: _sample_space(a, self.tick, self._observation_space) for a in self.agents}
    ...
    def step(self, actions):
        ...
        for kill in dead:
            ...
            obs[kill] = _sample_space(kill, self.tick, self._observation_space, zero=True)
            rewards[kill] = -1
            dones[kill] = True
