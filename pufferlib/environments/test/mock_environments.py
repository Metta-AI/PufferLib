...
class TestEnv(ParallelEnv):
    def __init__(self, observation_space, action_space, initial_agents, max_agents, spawn_per_tick, death_per_tick):
    ...
    def reset(self, seed=None):
        ...
        obs = {a: _sample_space(a, self.tick, self.single_observation_space) for a in self.agents}
...
def make_mock_multiagent_env(
    observation_space, action_space, initial_agents, max_agents, spawn_per_tick, death_per_tick, homogeneous_spaces=True
):
    return partial(
        TestEnv,
        observation_space=observation_space,
