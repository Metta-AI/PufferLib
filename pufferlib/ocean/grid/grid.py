...
class PufferGrid(pufferlib.PufferEnv):
    def __init__(self, render_mode="rgb_array", vision_range=3, num_envs=4096, report_interval=1024):
        ...
        self.buf = pufferlib.namespace(
            observations=np.zeros((num_envs, self.obs_size * self.obs_size + 3), dtype=np.uint8),
    ...
    def reset(self, seed=0):
        if self.cenv is None:
            self.cenv = CGrid(self.buf.observations, self.actions, self.buf.rewards, self.dones, self.num_agents)
        self.cenv.reset()
    ...
    def step(self, actions):
        ...
        return (self.buf.observations, self.buf.rewards, self.buf.terminals, self.buf.truncations, infos)
