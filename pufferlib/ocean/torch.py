...
class Snake(nn.Module):
    def __init__(self, env, cnn_channels=32, hidden_size=128, **kwargs):
        ...
        self.network = nn.Sequential(
            pufferlib.pytorch.layer_init(nn.Conv2d(8, cnn_channels, 5, stride=3)),
            nn.ReLU(),
...
class Grid(nn.Module):
    def __init__(self, env, cnn_channels=32, hidden_size=128, **kwargs):
        ...
        self.cnn = nn.Sequential(
            pufferlib.pytorch.layer_init(nn.Conv2d(7, cnn_channels, 5, stride=3)),
            nn.ReLU(),
            pufferlib.pytorch.layer_init(nn.Conv2d(cnn_channels, cnn_channels, 3, stride=1)),
            nn.Flatten(),
        )
        ...
        if self.is_continuous:
            ...
            self.actor = pufferlib.pytorch.layer_init(nn.Linear(hidden_size, 6), std=0.01)
