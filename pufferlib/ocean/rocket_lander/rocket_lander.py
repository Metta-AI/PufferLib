...
class RocketLander(pufferlib.PufferEnv):
    def __init__(self, num_envs=1, render_mode=None, report_interval=32, buf=None):
        self.single_observation_space = gymnasium.spaces.Box(low=0, high=1, shape=(6,), dtype=np.float32)
        ...
        self.c_envs = CyRocketLander(
            self.observations, self.float_actions, self.rewards, self.terminals, self.truncations, num_envs
        )
...
def test_performance(timeout=10, atn_cache=1024):
    ...
    print("SPS: %f", env.num_envs * tick / (time.time() - start))
