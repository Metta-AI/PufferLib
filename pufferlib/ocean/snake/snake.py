...
class Snake(pufferlib.PufferEnv):
    def __init__(
        ...
        valid_params = len(widths) == len(heights) == len(num_snakes) == len(num_food) == len(leave_corpse_on_death)
        if not valid_params:
        ...
        max_area = max([w * h for h, w in zip(heights, widths, strict=False)])
...
def test_performance(timeout=10, atn_cache=1024):
    ...
    print("SPS: %f", total_snakes * tick / (time.time() - start))
