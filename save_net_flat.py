...
def test_model(model):
    ...
    obs_flat = 26
    x = (
        torch.arange(0, batch_size * (obs_window * obs_window * obs_window_channels + obs_flat)).reshape(batch_size, -1)
        % 16
    )

    cnn_features = x[:, :-obs_flat].view(batch_size, obs_window, obs_window, obs_window_channels).long()
    ...
    cnn_features = torch.from_numpy(
        np.arange(batch_size * 11 * 11 * 19).reshape(batch_size, 19, obs_window, obs_window)
    ).float()
    ...
    print(
        "encode decode weight and bias sum:",
        model.encoder.weight.sum() + model.encoder.bias.sum() + model.decoder.weight.sum() + model.decoder.bias.sum(),
    )
    print("X sum:", x.sum())
...
def test_lstm():
    ...
    buffer = (torch.matmul(input, weights_input.T) + bias_input + torch.matmul(h, weights_state.T) + bias_state)[0]
