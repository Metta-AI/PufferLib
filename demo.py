...
def sweep_carbs(args, env_name, make_env, policy_cls, rnn_cls):
    ...
    import numpy as np
    from carbs import CARBS, CARBSParams, LinearSpace, LogitSpace, LogSpace, ObservationInParam, Param
    ...
    if "env" in sweep_parameters:
        ...
        if "reward_death" in env_params:
            param_spaces.append(carbs_param("env", "reward_death", "linear", sweep_parameters, search_center=-0.42))
        if "reward_xp" in env_params:
            param_spaces.append(
                carbs_param("env", "reward_xp", "linear", sweep_parameters, search_center=0.015, scale=0.05)
            )
        if "reward_distance" in env_params:
            param_spaces.append(
                carbs_param("env", "reward_distance", "linear", sweep_parameters, search_center=0.15, scale=0.5)
            )
        if "reward_tower" in env_params:
            param_spaces.append(carbs_param("env", "reward_tower", "linear", sweep_parameters, search_center=4.0))

        # Atari
        if "frameskip" in env_params:
            param_spaces.append(
                carbs_param("env", "frameskip", "linear", sweep_parameters, search_center=4, is_integer=True)
            )
        if "repeat_action_probability" in env_params:
            param_spaces.append(
                carbs_param("env", "repeat_action_probability", "logit", sweep_parameters, search_center=0.25)
            )
    ...
    param_spaces += [
        ...
        carbs_param("train", "update_epochs", "linear", sweep_parameters, search_center=1, scale=3, is_integer=True),
        ...
        carbs_param("train", "batch_size", "log", sweep_parameters, search_center=default_batch, is_integer=True),
        carbs_param(
            "train", "minibatch_size", "log", sweep_parameters, search_center=default_minibatch, is_integer=True
        ),
        carbs_param("train", "bptt_horizon", "log", sweep_parameters, search_center=16, is_integer=True),
    ]
    ...
    def main():
        ...
        try:
            stats, uptime, new_elos, vecenv["vecenv"] = train(
                args, make_env, policy_cls, rnn_cls, wandb, elos=elos, vecenv=vecenv["vecenv"] if cache_vecenv else None
            )
...
def train(
    ...
    if subprocess:
        ...
        queue = Queue()
        p = Process(target=train, args=(args, make_env, policy_cls, rnn_cls, wandb, eval_frac, elos, False, queue))
        p.start()
        p.join()
    ...
    if args["vec"] == "serial":
        ...
        raise ValueError("Invalid --vec (serial/multiprocessing/ray/native).")
    ...
    train_config = pufferlib.namespace(
        **args["train"], env=env_name, exp_id=args["exp_id"] or env_name + "-" + str(uuid.uuid4())[:8]
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--env", "--environment", type=str, default="puffer_squared", help="Name of specific environment to run"
    )
    parser.add_argument(
        "--mode", type=str, default="train", choices="train eval evaluate sweep sweep-carbs autotune profile".split()
    )
    parser.add_argument(
        "--vec-overwork", action="store_true", help="Allow vectorization to use >1 worker/core. Not recommended."
    )
    parser.add_argument("--eval-model-path", type=str, default=None, help="Path to a pretrained checkpoint")
    parser.add_argument("--baseline", action="store_true", help="Load pretrained model from WandB if available")
    parser.add_argument(
